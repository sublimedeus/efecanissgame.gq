...
const Terminal = ({ theme, setTheme }) => {
  ...
  const handleClose = () => window.location.href = 'https://instagram.com/efecan_755';
  const handleMinMax = () => {
...
class Field extends React.Component {
  ...
  handleInputExecution(cmd, params = [], flags = []) {
    if (cmd === 'help') {
      ...
      () => window.open('https://instagram.com/efecan_755'));
    } else if (cmd === 'theme') {
      ...
      return window.location.href = 'https://instagram.com/efecan_755';
    } else if (cmd === 'time') {
      return this.setState(state => ({
      ...
      return this.setState(state => ({
        fieldHistory: [...state.fieldHistory, { text: [
          'Email: *********@.com',
          'Website: deus-forum.gq',
          'LinkedIn: @***',
          'GitHub: @******',
          'İnstagram: @efecan_755'],
          hasBuffer: true }] }));
      ...
      if (params.length === 1) {
        const projects = [{
          title: 'minesweeper',
          live: 'https://instagram.com/efecan_755/' },
        {
          title: 'puniurl',
